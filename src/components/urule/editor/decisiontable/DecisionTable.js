...
//			$(".htCore th").css("background-color","rgb(223, 222, 203)")
  ...
  URule.DecisionTable.prototype = {
    ...
    load: function(callback) {
      ...
      var decisionTable = {
        'rows': [
          { 'num': 0, 'height': 40 }, { 'num': 1, 'height': 40 }
        ],
        'columns': [
          { 'num': 0, 'width': 150, 'type': 'Criteria' },
          { 'num': 1, 'width': 150, 'type': 'Criteria' },
          { 'num': 2, 'width': 200, 'type': 'ExecuteMethod' }
        ],
        'cellMap': {
          '0,0': {
            'row': 0,
            'col': 0,
            'rowspan': 1,
            'joint': { 'type': 'and', 'junction': { 'junctionType': 'and' } }
          },
          '0,1': {
            'row': 0,
            'col': 1,
    ...
    initMenu: function() {
      ...
      const setCustomCondition = () => {
        const col = self.getHighlight().col

        const column = self.getColData(col)
        window.showTableMenu((e) => {
          column.variableCategory = 'custom'
          column.variableLabel = e.variableCategory
          column.variableName = e.variableCategory
          column.datatype = 'InOut'
          self.setDirty()
          self.invoke('render')
        })
      }
      ...
      criteriaConfig.menuItems.push(customCondition)
      var actionConfig = {
        onShow: onShow,
        menuItems: []
      }
